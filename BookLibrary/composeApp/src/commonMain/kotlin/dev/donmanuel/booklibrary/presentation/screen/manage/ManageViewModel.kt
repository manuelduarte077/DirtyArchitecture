package dev.donmanuel.booklibrary.presentation.screen.manage
...
class ManageViewModel(
    private val database: BookDatabase, savedStateHandle: SavedStateHandle
) : ViewModel() {
    ...
    init {
        viewModelScope.launch {
            if (selectedBookId != -1) {
                val selectedBook = database.bookDao().getBookById(selectedBookId)
    ...
    fun insertBook(
        ...
        viewModelScope.launch {
            try {
                if (titleField.value.isNotEmpty() && summaryField.value.isNotEmpty() && categoryField.value.isNotEmpty() && tagsField.value.isNotEmpty() && authorField.value.isNotEmpty()) {
